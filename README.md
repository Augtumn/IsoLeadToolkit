...
- HDBSCAN 聚类分析: `uv pip install -e ".[hdbscan]"`
...
│   ├── overlay_state.py             # 覆盖层状态
│   └── state/                       # 状态管理（StateStore + Gateway + Views）
├── data/
│   ├── loader.py                    # 数据加载与清洗
│   └── geochemistry/                # 地球化学计算模块
├── plugins/                         # 插件系统
│   ├── api.py                       # 插件接口定义
│   ├── manager.py                   # 插件管理器
│   ├── registry.py                  # 全局注册表
│   ├── builtins/                    # 内置插件（混合/端元/聚类/溯源ML/子集）
│   └── examples/                    # 第三方插件模板
├── ui/
│   ├── app.py                       # 应用生命周期
...
│   ├── panels/                      # 6 个面板（data/display/analysis/export/legend/geo）
│   ├── dialogs/                     # 专用对话框
│   └── widgets.py                   # 可复用组件库
├── visualization/
│   ├── events.py                    # 交互事件编排
│   ├── embedding_worker.py          # 异步嵌入计算
