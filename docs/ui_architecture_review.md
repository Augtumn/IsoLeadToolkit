...
| 项 | 内容 | 状态 |
|---|---|---|
| **A** | 声明式状态字段注册表 | **已完成**：`core/state/fields.py` 驱动 **146 个字段**（默认值 + state→snapshot 强转 + snapshot→state 拷贝 + holder；带参强转用 `functools.partial` 表达）；store 两个字典各只剩一处 `**` 解包、`_normalizers` 只剩一次 `sync_fields()`；**53 个仍显式且全部有据**（40 个瞬态运行时不持久化；13 个两侧拷贝语义不同，如 `kde_thresh`/`kde_bw_method`），由守卫 `check_state_field_coverage.py` 强制登记；`check_state_sync_coverage.py` 扰动 174 字段无缺口 |
