...
| 项 | 内容 | 状态 |
|---|---|---|
| **A** | 声明式状态字段注册表 | **进行中**：`core/state/fields.py` 现驱动 **112 个字段**（默认值 + state→snapshot 强转 + snapshot→state 拷贝 + holder，其中 29 个用 `functools.partial` 表达带参强转）；store 的两个字典各只剩一处 `**` 解包，`_normalizers` 只剩一次 `sync_fields()`；**87 个仍显式并有据**（40 个瞬态运行时不持久化、47 个无初始快照默认值，需注册表支持「仅投影/回写侧生效」）；守卫 `check_state_field_coverage.py` 强制每个快照字段要么已注册、要么在允许清单里注明原因；`check_state_sync_coverage.py` 扰动 174 字段无缺口；`tests/test_state_fields.py` 覆盖四个消费者一致 |
