...
- **模式年龄（age.py）**：论文式 (12) 的残差 `albarede_model_age_residual()` 与求解器 `calculate_albarede_model_age()`——端点括根 + 区间扫描，区间 (0, T0)，无解返回 NaN/None。
...
- **集成入口（`__init__.py`）**：`calculate_albarede_parameters()` 一站式返回 T/μ/κ/ω/Δμ/Δκ（键名常量化）；**刻意不并入** `calculate_all_parameters()`——两者参考组成与年龄口径不同，混在同一字典会让下游误用错参考。
