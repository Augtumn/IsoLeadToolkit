...
- 在显示面板顶部新增“快速样式条”：`网格`、`标题`、`标记大小`、`透明度`、`DPI`。
- 快速项变更优先走轻量刷新（`refresh_plot_style`），避免频繁全重绘。

#### F2 预设预览与对比（高优先级）

- 主题下拉增加“预览”按钮：临时应用，不写入会话。
- 增加“对比当前/预设差异”提示（关键字段列表），降低误覆盖风险。

#### F3 颜色输入可视化（中优先级）

- 为颜色输入框增加颜色按钮与合法性校验（HEX/RGB），无效值高亮提示。
- 颜色变更支持立即预览，失败时回退到上一个有效颜色。

#### F4 参数搜索与折叠记忆（中优先级）

- 新增设置搜索框（按标签关键字过滤可见项）。
- 记忆上次展开分段与滚动位置，提升重复调参效率。

#### F5 样式导入导出（低优先级）

- 主题支持导出/导入 JSON（跨机器共享）。
- 增加 schema 校验与版本字段，保证兼容性。
