...
**实现细节:** 第三项的指数因子取 ²³⁸U 项 (e^{λT_i} − 1)。**论文 p.857 式 (12) 的
印刷版分子写作 e^{λ'T_i} − 1（²³⁵U，带撇号；已用 MinerU 页面图像放大复核排版）**，
但由式 (11) 的 x/y 生长方程精确消去 Δμ_i 必须得到 ²³⁸U 项，因为
`s(T_i, 0)·(e^{λT_i} − 1) = (e^{λ'T_i} − 1)/137.88`。

数值后果（本仓库可复现，见 `tests/test_geochemistry_albarede.py`）：对由
T_i = 300 Ma、μ_i = 9.90、κ_i = 4.05 正演得到的合成样品，

| 形式 | f(300 Ma) | 数值根 |
|------|-----------|--------|
| 印刷版（λ'） | +6.62 | ≈ 47.6 Ma（与式 (11) 矛盾） |
| 本实现（λ） | 3.6 × 10⁻¹⁵ | 300.0000 Ma |
...
**实现细节:** 论文 p.858 式 (15) 的印刷版把 (14) 中的 `(μ_i Δκ_i + κ* Δμ_i)(…)`
原样留在右端分子（已用页面图像放大复核），代入后两侧的 `μ_i Δκ_i` 相消：

```
右端 − Δκ_i = Δκ_i^true + 2κ* Δμ_i / μ_i = 常数(与试探 Δκ_i 无关)
```
