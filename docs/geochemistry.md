...
**关于 z\***：ASTR 源码写作 38.86，而 SilverQuest_v1 随附的矿石库
（`Pb_DB_20240310AllGalenas.xlsx`，6938 条，其 `Tmod/mu/kappa` 三列即 AJ84 产物）
只有在取 38.83 时才能被复现——κ 残差 1e-5，用 38.86 则 κ 系统性偏 −0.016（≈0.4%）。
故取 38.83（亦为 2012 版印刷值）。
...
`(1/U)·(λ'/λ)·e^{(λ'−λ)T}`，使"与锚点同龄"的样品仍有有限残差。
...
**求解区间差异（有意为之）**：本实现限定 T_i ∈ (0, T0)，无解返回 NaN；ASTR 的无界
Newton 对模型族外数据会给出非物理根——SilverQuest 矿石库中就有 `Tmod = −84 Ma`
的行（本仓库测试 `test_out_of_family_sample_returns_nan_instead_of_a_negative_age`
固定了这一差异）。
...
用 ASTR 的 AJ84 常数复算 SilverQuest 矿石库 6938 条数据（其 `Tmod/mu/kappa` 即 AJ84 输出）：

| 量 | 中位偏差 | p95 \|偏差\| | 备注 |
|----|----------|--------------|------|
| T | −0.002 Ma | 0.24 Ma | 库中 T 仅保留 3 位小数 |
| μ | −1 × 10⁻⁵ | 2 × 10⁻⁴ | |
| κ | +1 × 10⁻⁵ | 3 × 10⁻² | z\* = 38.83 时；38.86 则偏 −0.016 |

反推检验：把 `μ_i·D − x_i`（D = e^{λT0} − e^{λT_i}）作为应恒定的量，在 6938 条数据上
IQR = 5 × 10⁻⁵，说明这批数据确实出自同一套 AJ84 参考代数。
`tests/test_geochemistry_albarede.py` 固定了其中 3 行作为回归样本。
